...
工作流内容：
- 验证构建（`make doctor`、前端 `pnpm run build`、Go 测试 `go test -v ./...`）
...
- 工作流使用 `Go 1.25` 与 `Node.js 20 + pnpm 10`，确保依赖版本兼容。
...
- 课程资源：检测课程索引与可用性。
- 进度文件：检测 `data/progress.json` 的读写、结构与语义一致性。
- 进程文件：检测 `tmp/kwdb-playground.pid` 是否可读且内容合法。
- 可执行文件位置：优先基于 `tmp/kwdb-playground.pid`，若缺失则回退按监听端口定位正在运行的 `kwdb-playground` 进程可执行文件。
...
kwdb-playground doctor --host localhost --port 3006

# 自动修复可修复项（执行修复并在结束后输出修复后诊断）
kwdb-playground doctor --fix

# 仅预览修复动作（不执行修复）
kwdb-playground doctor --fix --fix-scope docker --dry-run
```
...
```
KWDB Playground 环境诊断报告
------------------------------------------------------------
结果: 通过 (8/8)  |  时间: 2026-03-10 10:19  |  版本: 1.1.0

● Docker 环境
  └─ 连接状态: 已连接（API v1.53，要求 ≥ v1.41）
...
● 端口占用状态 (0.0.0.0:3006)
  └─ 服务状态: 服务已启动（kwdb-playground）

● 课程加载与完整性
  └─ 统计: 课程加载成功，共 8 门，数据完整性检查通过

● 进度数据
  └─ 记录: progress.json 校验通过，共 1 条进度记录

● 进程管理 (PID: 2016)
  └─ 文件: /Users/guoxudong/kwdb/playground/tmp/kwdb-playground.pid

● 可执行文件路径
  └─ 路径: /Users/guoxudong/kwdb/playground/bin/kwdb-playground

● 服务健康检查 (0.0.0.0:3006)
  └─ 状态: 服务正在运行且健康（/health 返回 200）

------------------------------------------------------------
✨ 结论: 环境健康，开启你的 KWDB Playground 吧。
```
...
这与 `kwdb-playground doctor` 命令的功能一致，方便用户在界面中快速诊断问题。

---

## 镜像管理页面

镜像管理页面位于导航栏「镜像管理」，用于集中处理课程镜像相关操作。
...
- 页面顶部镜像源选择器为全局入口
- 预拉取、清理、诊断三分区均使用当前选中镜像源
- 选择自定义镜像源时，需要填写前缀后再执行操作
...
- Docker 相关功能不可用
  - 先执行 `kwdb-playground doctor` 查看 Docker 环境项；
