...
- **💻 丰富交互**: 支持 Shell、SQL 和 Python 代码执行等多种终端模式，满足不同类型的课程需求。
- **⏸️ 进度管理**: 支持学习进度保存与环境状态的暂停/恢复（v0.5.0 新增）。
- **🌍 智能镜像加速**: 支持多 Docker 镜像源智能选择与连通性测试（v0.4.1 新增）。
- **🐍 Python 代码执行**: 支持在课程中编写和运行 Python 代码，实时查看执行结果（v0.6.0 新增）。
...
- **Code 终端型**：在浏览器内编写代码并实时执行，支持多种语言（如 Python、JavaScript），适合编程学习。
...
#### 镜像源智能选择与测速
自动检测并支持 Docker Hub、ghcr.io、阿里云镜像源，启动前验证连通性并自动保存您的偏好配置。
...
#### 引导教程
首次访问各页面时，可通过导航栏的帮助按钮开启交互式引导，快速了解各项功能。
...
- 🐍 **代码执行功能**：[`docs/code-execution.md`](./docs/code-execution.md)（Python 代码执行功能说明）
