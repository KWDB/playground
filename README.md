...
- **暂停状态检测**：重新打开课程时自动检测暂停的容器
- **进度恢复**：可以恢复之前暂停的容器，继续学习
- **状态指示**：清晰显示容器当前状态（运行中、已暂停、已停止等）

这一功能适合耗时较长的实战课程。当容器处于暂停状态时，可以通过"恢复容器"按钮继续学习，环境状态和进度都会被保留。

> **使用方式**：可通过 API 接口 `POST /api/containers/:id/pause` 暂停容器，或使用 Docker 命令行工具。暂停的容器会在重新打开课程时被检测到并可恢复。
